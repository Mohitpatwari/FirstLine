...
import { uploadOnCloudinary, deleteFromCloudinary } from "../utils/cloudinary.js";
import { OTP } from "../models/otp.model.js";
import { sendOTPSMS } from "../utils/otpSMS.js"; // This now handles our Nodemailer emails
import jwt from "jsonwebtoken";
...
const generateAccessAndRefreshToken = async (userId) => {
  try {
    ...
    throw new ApiError(error.statusCode || 500, "Something went wrong while generating tokens");
  }
}
...
export const registerUser = asyncHandler(async (req, res) => {
  ...
  const createdUser = await User.findById(user._id).select(
    "-refreshToken -location -password"
  );
...
export const loginUser = asyncHandler(async (req, res) => {
  ...
  await sendOTPSMS(cleanEmail, otp);
...
export const updateProfile = asyncHandler(async (req, res) => {
  ...
  if (phone) updates.phone = phone; // Keeping phone optional for profile updates
  if (skills)
    updates.skills = Array.isArray(skills) ? skills : JSON.parse(skills);
...
export const addGuardian = asyncHandler(async (req, res) => {
  ...
  const guardian = await User.findOne({ email: normalizeEmail(guardianEmail) });
...
export const sendOTP = asyncHandler(async (req, res) => {
  ...
  if (!user) {
    // Create a new user with email (without password initially)
    user = await User.create({
      email: cleanEmail,
      fullName: "New User",
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, { message: "OTP sent to email successfully" }, "OTP sent"));
});
...
export const verifyOTP = asyncHandler(async (req, res) => {
  ...
  // Mark as verified and clear OTP
  user.isEmailVerified = true; // Was isPhoneVerified
  user.otp = null;
  user.otpExpiry = null;
  user.lastSeen = new Date();
  await user.save({ validateBeforeSave: false });
...
export const adminListUsers = asyncHandler(async (req, res) => {
  ...
  const users = await User.find({ role: { $ne: "admin" } })
    .select("fullName email phone avatar trustScore volunteerRating skills isSuspended totalResponses successfulResponses falseAlertCount role createdAt lastSeen")
    .sort({ createdAt: -1 });
