...
//   console.log(
//     `[VolunteerSuggestions] Incident ${incident._id} type=${incident.type} coords=[${incidentLng},${incidentLat}] radius=${incident.radiusMeters}m — found ${candidates.length} candidate(s)`,
//   );
...
//     const [candidateLng, candidateLat] = candidate.location?.coordinates || [
//       0, 0,
//     ];
...
//     const normalizedVolunteerRating =
//       Number(candidate.volunteerRating || 50) / 100;
...
//     const llmSelected = scored.find(
//       (item) => String(item.candidate._id) === selectedId,
//     );
...
//     if (
//       Array.isArray(llmOutput?.rankedCandidateIds) &&
//       llmOutput.rankedCandidateIds.length
//     ) {
//       const validSet = new Set(
//         scored.map((item) => String(item.candidate._id)),
//       );
...
//       (() => {
//         const ratingInFive = Math.max(
//           0,
//           Math.min(5, Number(item.candidate.volunteerRating || 0) / 20),
//         );
//         const selectionFactor =
//           ratingInFive > 0
//             ? Number((item.distanceKm / ratingInFive).toFixed(4))
...
//     .sort((a, b) => {
//       const factorDiff =
//         Number(a.selectionFactor || Number.POSITIVE_INFINITY) -
//         Number(b.selectionFactor || Number.POSITIVE_INFINITY);
//       if (factorDiff !== 0) return factorDiff;

//       const ratingDiff =
//         Number(b.volunteerRating || 0) - Number(a.volunteerRating || 0);
...
//   console.log(
//     `[VolunteerSuggestions] Returning ${suggestedVolunteers.length} volunteers — top: ${suggestedVolunteers[0]?.fullName || "none"} (${suggestedVolunteers[0]?.distanceKm}km)`,
//   );
...
//   const severity =
//     typeLower === "health" || typeLower === "fire"
//       ? "high"
//       : typeLower === "road"
//         ? "medium"
//         : "low";
...
//         isSuspended: false,
//         skills: {
//           $elemMatch: { $regex: new RegExp(relevantSkills.join("|"), "i") },
//         },
//         location: {
//           $near: {
...
//       })
//         .select(
//           "fullName phone skills trustScore volunteerRating location avatar",
//         )
//         .limit(10);
...
//             phones: [best.phone],
//             message: `FIRSTLINE AUTO-DISPATCH: You have been assigned to a ${type.toUpperCase()} incident. ${String(description || "Emergency help needed").slice(0, 80)}. Open app to navigate.`,
//           });
//         }
//       } catch {}
//     }
...
//   return res
//     .status(200)
//     .json(new ApiResponse(200, incidents, "Open incidents fetched"));
// });
...
//   return res
//     .status(200)
//     .json(new ApiResponse(200, incidents, "Nearby incidents fetched"));
// });
...
//         200,
//         populated,
//         alreadyResponding ? "Already responding" : "Responder joined",
//       ),
//     );
// });
...
//     return res
//       .status(200)
//       .json(new ApiResponse(200, incident, "Incident already resolved"));
//   }
...
//   return res
//     .status(200)
//     .json(new ApiResponse(200, incident, "Incident marked resolved"));
// });
...
//   const { recommendedVolunteer, suggestedVolunteers, reason, evaluatedCount } =
...
//   const alertMessage = `FIRSTLINE SOS: ${incident.type.toUpperCase()} incident near you. Details: ${String(
...
import { asyncHandler } from "../utils/asyncHandler.js";
import { runLLMJson } from "../utils/llm.js";
import { sendSOSAlert } from "../utils/alertSMS.js"; // Updated to the new Email utility
import { getIO, joinUserToRoom } from "../socketInstance.js";
...
const computeVolunteerSuggestions = async (incident) => {
  ...
  }).select("fullName email trustScore volunteerRating skills location"); // Changed phone to email
    ...
    scored.map((item) => [
      ...
      (() => {
        ...
        return {
          _id: item.candidate._id,
          fullName: item.candidate.fullName,
          email: item.candidate.email, // Changed from phone to email
          volunteerRating: item.candidate.volunteerRating,
          trustScore: item.candidate.trustScore,
          skills: item.candidate.skills || [],
          distanceKm: Number(item.distanceKm.toFixed(2)),
  ...
  try {
    ...
    if (relevantSkills.length > 0) {
      const skilledUsers = await User.find({
        ...
        .select(
          "fullName email skills trustScore volunteerRating location avatar", // Changed phone to email
        )
        .limit(10);
      ...
      skilledRespondersInfo = skilledUsers.map((u) => {
        ...
        return {
          _id: u._id,
          fullName: u.fullName,
          email: u.email, // Changed phone to email
          skills: u.skills,
          trustScore: u.trustScore,
          volunteerRating: u.volunteerRating,
          avatar: u.avatar,
  ...
  try {
    ...
    if (best && best._id) {
      ...
      incident.autoDispatch = {
        volunteerId: best._id,
        volunteerName: best.fullName || "",
        volunteerEmail: best.email || "", // Changed to volunteerEmail
        distanceKm: best.distanceKm ?? null,
        rating: best.volunteerRating ?? null,
        dispatchedAt: new Date(),
        status: "dispatched",
      ...
      // Send Email alert to dispatched volunteer
      try {
        if (best.email) {
          await sendSOSAlert(best.email, {
            type: type,
            description: description,
          });
        }
      } catch (err) {
        console.error("Auto-dispatch email failed", err);
      }
    }
  } catch (err) {
    console.error("[AutoDispatch] Could not auto-dispatch:", err.message);
...
export const listOpenIncidents = asyncHandler(async (_, res) => {
  const incidents = await Incident.find({ status: "open" })
    .populate("createdBy", "fullName email") // Changed phone to email
    .populate("responders.userId", "fullName email") // Changed phone to email
    .sort({ createdAt: -1 });
  return res
    .status(200)
    .json(new ApiResponse(200, incidents, "Open incidents fetched"));
...
export const adminIncidentSummary = asyncHandler(async (_, res) => {
  const [openCount, resolvedCount, totalCount, recent] = await Promise.all([
    ...
    Incident.countDocuments(),
    Incident.find()
      .populate("createdBy", "fullName email avatar trustScore") // Changed phone to email
      .populate("responders.userId", "fullName email avatar") // Changed phone to email
      .populate("resolvedBy", "fullName email") // Changed phone to email
      .sort({ createdAt: -1 })
      .limit(100),
  ]);
...
export const notifySuggestedVolunteers = asyncHandler(async (req, res) => {
  ...
  // Pull emails instead of phones
  const emails = selectedVolunteers.map((item) => item.email).filter(Boolean);
    ...
    return res.status(200).json(
      new ApiResponse(
        200,
        {
          ...
          suggestedVolunteers: selectedVolunteers,
          message: selectedVolunteerId
            ? "Selected volunteer is not available for Email alerts"
            : "No valid volunteer email addresses found",
        },
        "Volunteer email notification skipped",
      ),
    );
  }

  // Send SOS emails concurrently to all matched volunteers
  try {
    await Promise.all(
      emails.map((email) =>
        sendSOSAlert(email, {
    ...
    console.error("Failed to notify some volunteers", error);
  }
  ...
  return res.status(200).json(
    new ApiResponse(
      ...
      "Email alerts sent to suggested volunteers",
    ),
  );
});