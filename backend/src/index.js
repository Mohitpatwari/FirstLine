...
// import { sendSOSAlert } from "./utils/alertSMS.js";
...
//         });
//         console.log(
//           `📍 DB location updated for user ${userId}: [${lng}, ${lat}]`,
//         );
//       } catch (err) {
...
//           candidates.sort(
//             (a, b) => (b.volunteerRating || 0) - (a.volunteerRating || 0),
//           );
...
//                 phones: [best.phone],
//                 message: `FIRSTLINE AUTO-DISPATCH: You are assigned to a ${type.toUpperCase()} incident. Open app to navigate.`,
//               });
//             }
//           } catch {}
//         }
...
//         console.error(
//           `❌ Port ${PORT} is already in use. Please run: lsof -ti:${PORT} | xargs kill -9`,
//         );
//         process.exit(1);
//       } else {
...
import { setIO } from "./socketInstance.js";
import { registerUserSocket, removeUserSocket } from "./socketInstance.js";
import { sendSOSAlert } from "./utils/alertSMS.js"; // Correct import
import path from "path";
import { fileURLToPath } from "url";
      ...
      try {
        ...
        if (candidates.length) {
          ...
          incident.autoDispatch = {
            volunteerId: best._id,
            volunteerName: best.fullName || "",
            volunteerEmail: best.email || "", // Changed phone to email
            distanceKm: Number(distanceKm.toFixed(2)),
            rating: best.volunteerRating || 50,
            dispatchedAt: new Date(),
            status: "dispatched",
          ...
          // Send Email alert
          try {
            if (best.email) { // Changed phone to email
              await sendSOSAlert(best.email, {
                type: type,
                description: description || "Emergency help needed",
              });
            }
          } catch (emailErr) {
            console.error("Socket auto-dispatch email failed", emailErr);
          }
        }
      } catch (dispatchErr) {
        console.error("[Socket AutoDispatch]", dispatchErr.message);
