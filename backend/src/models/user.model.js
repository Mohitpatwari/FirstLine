...
const userSchema = new mongoose.Schema(
  {
    ...
    password: {
      type: String,
      required: false, // Optional if using passwordless OTP login exclusively
    },
    avatar: {
      type: String,
      default: process.env.DEFAULT_AVATAR_URL,
...
userSchema.pre("save", async function () {
  ...
  this.password = await bcrypt.hash(this.password, Number(process.env.ENCRYPTION_ROUND || 10));
});
