...
function UserDashboard({ onLogout }) {
  ...
  const [latestIncidentSuggestion, setLatestIncidentSuggestion] =
    useState(null);
  ...
  useEffect(() => {
    ...
    incidents.forEach((inc) => {
      ...
      const isCreator = String(inc.createdBy?._id || inc.createdBy) === uid;
  ...
  useEffect(() => {
    ...
    const onIncidentUpdated = (updatedIncident) => {
      ...
      setIncidents((prev) => {
        ...
        if (exists) {
          return prev.map((inc) =>
            (inc._id || inc.id) === id ? updatedIncident : inc,
          );
        }
  ...
  const fetchAssessmentQuestions = async (type = assessmentType) => {
    ...
    try {
      ...
      const phoneQuery = parsedUser?.phone
        ? `&volunteerPhone=${encodeURIComponent(parsedUser.phone)}`
        : "";
  ...
  const handleRespond = async (incidentId) => {
    ...
    try {
      ...
      if (updated) {
        ...
        setIncidents((prev) =>
          prev.map((inc) =>
            (inc._id || inc.id) === incidentId ? updated : inc,
          ),
        );
      }
  ...
  const handleResolve = async (incidentId) => {
    ...
    try {
      ...
      setIncidents((prev) =>
        prev.filter((inc) => (inc._id || inc.id) !== incidentId),
      );
    } catch {
    } finally {
      setLoadingId("");
    ...
    try {
      ...
      setAssessmentStatus(
        error.response?.data?.message ||
          "AI rating failed. Please login again and retry.",
      );
    } finally {
      setAssessmentLoading(false);
    }
  ...
  const suggestBestVolunteer = async (incidentId) => {
    ...
    try {
      const response = await axiosInstance.get(
        `/incidents/${incidentId}/best-volunteer`,
      );
  ...
  const totalResponders = useMemo(
    () =>
      incidents.reduce(
        (total, incident) => total + (incident.responders?.length || 0),
        0,
      ),
    [incidents],
  );
      ...
      <section className="dashboard-shell user-dashboard-shell">
        <header className="dashboard-topnav user-topnav">
          ...
          <div className="dashboard-topnav-right">
            <span className="sos-pill">
              <strong>Active SOS:</strong> {incidents.length}
            </span>
        ...
        <section className="dashboard-stats-row user-stats-row">
          <article className="stat-card stat-card--incidents">
            ...
            <p className="stat-meta">
              <strong>Open incidents</strong> around the city
            </p>
          </article>
          <article className="stat-card stat-card--responders">
            <div className="stat-head">
              <p className="stat-label">
                <strong>Responders Live</strong>
              </p>
            ...
            <p className="stat-meta">
              <strong>Users</strong> currently responding
            </p>
          </article>
          <article className="stat-card stat-card--eta">
            ...
            <h4>Live</h4>
            <p className="stat-meta">
              <strong>Real-time</strong> SOS feed
            </p>
          </article>
        </section>
        ...
        <section className="dashboard-main-grid user-dashboard-grid">
          <aside className="dashboard-panel filters-panel user-panel glass-card">
            ...
            <p className="panel-caption">
              <strong>Select</strong> incident categories
            </p>
            ...
            <p className="panel-caption">
              <strong>AI Volunteer Rating</strong>
            </p>
            ...
            {assessmentQuestions.map((item) => (
              <div className="report-field ud-question-field" key={item.id}>
                <span className="ud-question-label">
            ...
            {assessmentResult?.rating !== undefined ? (
              <div className="ud-rating-result mt-2.5">
                <strong>Your volunteer rating:</strong>{" "}
                <span className="ud-rating-value">
                  {assessmentResult.rating}/100
                </span>{" "}
                <span className="ud-rating-grade">
          ...
          <section className="dashboard-panel live-map-panel user-panel glass-card">
            ...
            <p className="panel-caption">
              <strong>Auto-updating</strong> incidents, resources &amp;
              responders
            </p>
            ...
            <div className="ud-nav-row">
              <button
                ...
                onClick={() => navigate("/skill-registry")}
              >
                <Award size={14} /> <strong>Skill Registry</strong>
              </button>
              <button
                type="button"
                className="ud-nav-btn ud-nav-btn--green"
            ...
            {latestIncidentSuggestion?.suggestedVolunteers?.length ? (
              <div className="ud-suggestion-box">
                <p className="responder-chat-title">
                  <strong>LLM Suggested Volunteers</strong> (Latest Incident)
                </p>
                {latestIncidentSuggestion.suggestedVolunteers.map((item) => (
                  <p className="responder-chat-line" key={item._id}>
                    <strong>{item.fullName}</strong> &bull; Rating{" "}
                    <strong>{item.volunteerRating}/100</strong> &bull;{" "}
              ...
              {latestIncidents.length === 0 ? (
                <div className="update-item ud-empty-state">
                  ...
                  <p className="update-text">
                    New SOS requests will show here instantly.
                  </p>
                </div>
              ) : (
                latestIncidents.map((incident) => (
                  <div
                    className="update-item ud-incident-card"
                    key={incident._id}
                  >
                    <div className="ud-incident-header">
                      <span
                        className={`ud-incident-badge ud-badge--${incident.type || "other"}`}
                      >
                        <strong>{incident.type?.toUpperCase()}</strong>
                      </span>
                      <span className="ud-responder-count">
                        <Activity size={12} />{" "}
                        <strong>{incident.responders?.length || 0}</strong>{" "}
                        responder
                        {(incident.responders?.length || 0) !== 1 ? "s" : ""}
                      </span>
                    </div>
                    <p className="update-text">
                      <strong>
                    ...
                    {bestVolunteerByIncident[incident._id] ? (
                      <div className="responder-chat-placeholder ud-volunteer-card">
                        <p className="responder-chat-title">
                          <strong>AI Recommended Volunteer</strong>
                        </p>
                        <p className="responder-chat-line">
                          <strong>
                            {bestVolunteerByIncident[incident._id].fullName}
                          </strong>{" "}
                          &bull; Rating{" "}
                          <strong>
                            {
                        ...
                        <p className="responder-chat-line">
                          ...
                          {bestVolunteerByIncident[incident._id].distanceKm} km
                          &bull; <strong>Trust</strong>{" "}
                          {bestVolunteerByIncident[incident._id].trustScore}/10
                        </p>
                      </div>
                    ) : null}
                    ...
                    {activeChatIncident === incident._id ? (
                      <div
                        className="responder-chat-placeholder ud-chat-box"
                        style={{
                          background: "#f8fafc",
                          borderRadius: 10,
                          padding: 14,
                      ...
                      >
                        <p
                          className="responder-chat-title"
                          style={{
                            display: "flex",
                            alignItems: "center",
                            gap: 6,
                        ...
                        >
                          ...
                          >
                            {incident.responders?.length || 0} in chat
                          </span>
                        </p>
                        <div
                        ...
                        >
                          {(chatMessages[incident._id] || []).length === 0 ? (
                            ...
                            >
                              No messages yet. Say something to coordinate!
                            </p>
                          ) : (
                            (chatMessages[incident._id] || []).map(
                              (msg, idx) => (
                                <div
                                  key={idx}
                                  style={{
                        ...
                        <div style={{ display: "flex", gap: 6 }}>
                          <input
                            ...
                            onKeyDown={(e) =>
                              e.key === "Enter" && sendChatMessage(incident._id)
                            }
                            style={{
                              flex: 1,
                              padding: "8px 12px",
        ...
        <footer className="ud-footer">
          <p>
            <strong>FirstLine</strong> &mdash; Community-powered emergency
            response
          </p>
        </footer>
      </section>
