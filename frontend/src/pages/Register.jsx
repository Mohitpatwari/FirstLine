...
//             <strong>Join FirstLine</strong> to help your community
//           </p>
...
function Register() {
  const navigate = useNavigate();
  const [name, setName] = useState("");
  const [email, setEmail] = useState(""); // Replaced phone with email
  const [avatar, setAvatar] = useState(null);
  const [loading, setLoading] = useState(false);
  const [status, setStatus] = useState({ type: "", message: "" });
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const formData = new FormData();
      formData.append("name", name.trim());
      formData.append("email", email.trim().toLowerCase()); // Appending email instead of phone
    ...
    <main className="auth-page">
      ...
      <section className="auth-body">
        <form className="auth-card" onSubmit={handleSubmit}>
          ...
          {/* Removed phone-input-wrap and +91 span */}
          <input
            id="register-email"
            className="text-input"
            type="email"
