        ...
        <section className="dashboard-stats-row user-stats-row">
          <article className="stat-card stat-card--incidents">
            ...
            <p className="stat-meta">
              <strong>Registered</strong> skills
            </p>
          </article>
          <article className="stat-card stat-card--responders">
            <div className="stat-head">
              <p className="stat-label">
                <strong>Community Responders</strong>
              </p>
            ...
            <p className="stat-meta">
              <strong>Skilled</strong> volunteers registered
            </p>
          </article>
          <article className="stat-card stat-card--eta">
            <div className="stat-head">
              <p className="stat-label">
                <strong>Nearby Skilled</strong>
              </p>
        ...
        <section className="dashboard-main-grid user-dashboard-grid">
          ...
          <div className="dashboard-panel user-panel glass-card">
            ...
            <p className="panel-caption">
              <strong>Select</strong> your skills or add custom ones
            </p>

            {Object.entries(skillOptions).map(([category, skills]) => (
              <div key={category} className="sr-category-block">
                <p
                  className="sr-category-title"
                  style={{ color: SKILL_COLORS[category] || "#333" }}
                >
                  <strong>
          ...
          <div className="dashboard-panel live-map-panel user-panel glass-card">
            <h3>
              <strong>Find Skilled Responders</strong>
            </h3>
            <p className="panel-caption">
              <strong>Search</strong> for skilled people near you
            </p>
              ...
              {nearbyResponders.length === 0 ? (
                <div className="update-item ud-empty-state">
                  ...
                  <p className="update-title">
                    <strong>No nearby responders yet</strong>
                  </p>
                  <p className="update-text">
                    Click &quot;Search Nearby&quot; to find skilled people
                    around you.
                  </p>
                </div>
              ) : (
                nearbyResponders.map((r) => (
                  <div className="sr-responder-card" key={r._id}>
                    <div className="sr-responder-header">
                      <strong className="sr-responder-name">
                        {r.fullName}
                      </strong>
                    ...
                    <div className="filter-chip-list sr-responder-skills">
                      {(r.skills || []).map((s) => (
                        <span
                          key={s}
                          className="filter-chip active sr-skill-mini"
                        >
                          {s}
                        </span>
                      ))}
                    ...
                    <div className="sr-responder-meta">
                      <span>
                        <Star size={12} /> <strong>Rating:</strong>{" "}
                        {r.volunteerRating}/100
                      </span>
                      <span>
                        <Award size={12} /> <strong>Trust:</strong>{" "}
                        {r.trustScore}/10
                      </span>
                    </div>
                  </div>
          ...
          <aside className="dashboard-panel user-panel glass-card ud-updates-panel">
            <h3>
              <strong>Skilled Volunteers</strong>
            </h3>
            <p className="panel-caption">
              <strong>Community</strong> leaderboard
            </p>
            <div className="update-list">
              {skilledUsers.length === 0 ? (
                <div className="update-item ud-empty-state">
                  ...
                  <p className="update-title">
                    <strong>No skilled volunteers yet</strong>
                  </p>
                  <p className="update-text">
                ...
                skilledUsers.slice(0, 10).map((u, idx) => (
                  <div className="update-item sr-leaderboard-row" key={u._id}>
                    <div className="ud-incident-header">
                      ...
                      <span className="sr-lb-rating">
                        {u.volunteerRating}/100
                      </span>
                    </div>
                    <div className="filter-chip-list sr-lb-skills">
                      {(u.skills || []).slice(0, 4).map((s) => (
                        <span key={s} className="filter-chip sr-skill-mini">
                          {s}
                        </span>
                      ))}
                      {(u.skills || []).length > 4 && (
                        <span className="filter-chip sr-skill-mini">
                          +{u.skills.length - 4}
                        </span>
                      )}
                    </div>
