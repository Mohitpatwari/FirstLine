  ...
  const misuseFlags = useMemo(
    () =>
      summary.incidents.filter(
        (inc) => inc.status === "open" && (inc.responders?.length || 0) === 0,
      ).length,
    [summary.incidents],
  );
  ...
  const handleToggleSuspend = async (userId) => {
    ...
    try {
      const res = await axiosInstance.patch(
        `/users/admin/${userId}/toggle-suspend`,
      );
    ...
    <main className="dashboard-page user-dashboard-page">
      <section className="dashboard-shell user-dashboard-shell admin-shell">
        ...
        <section className="dashboard-stats-row user-stats-row">
          <article className="stat-card stat-card--incidents">
            ...
            <p className="stat-meta">
              <strong>Live</strong> open incidents
            </p>
          </article>
          ...
          <article className="stat-card stat-card--responders">
            ...
            <p className="stat-meta">
              <strong>Total</strong> active responders
            </p>
          </article>
          ...
          <article className="stat-card stat-card--eta">
            ...
            <p className="stat-meta">
              <strong>Resolved</strong> cases total
            </p>
          </article>
          ...
          >
            ...
            <p className="stat-meta">
              <strong>{userStats.activeUsers}</strong> active &middot;{" "}
              <strong>{userStats.suspendedUsers}</strong> suspended
            </p>
          </article>
        </section>
          ...
          <section className="dashboard-main-grid user-dashboard-grid">
            ...
            <aside className="dashboard-panel filters-panel user-panel glass-card">
              ...
              <p className="panel-caption">
                <strong>Filter</strong> incidents by criteria
              </p>
              ...
              <div className="admin-trust-stats">
                <div className="admin-trust-item admin-trust-warning">
                  ...
                  <span>
                    <strong>Misuse flags pending:</strong> {misuseFlags}
                  </span>
                </div>
                <div className="admin-trust-item admin-trust-danger">
                  <AlertCircle size={14} />
                  <span>
                    <strong>Suspension candidates:</strong>{" "}
                    {misuseFlags > 2 ? misuseFlags - 2 : 0}
                  </span>
                </div>
              </div>
              ...
              <div className="coverage-tags">
                ...
                <span>
                  ...
                  <strong>Escalation first</strong> routing
                </span>
              </div>
            </aside>
            ...
            <section className="dashboard-panel live-map-panel user-panel glass-card">
              ...
              <p className="panel-caption">
                <strong>Control-room view</strong> with live incidents
              </p>
            ...
            <aside className="dashboard-panel user-panel glass-card ud-updates-panel">
              ...
              >
                <h3>
                  <strong>Incidents ({filteredIncidents.length})</strong>
                </h3>
              </div>
                ...
                {filteredIncidents.length === 0 ? (
                  <div className="update-item ud-empty-state">
                    ...
                    <p className="update-title">
                      <strong>No incidents match filters</strong>
                    </p>
                    <p className="update-text">
                      Adjust filters or wait for new incidents.
                    </p>
                  </div>
                ) : (
                      ...
                      >
                        ...
                        <div className="admin-incident-meta">
                          <span className="ud-responder-count">
                            <Activity size={12} />{" "}
                            <strong>{responders.length}</strong> responder
                            {responders.length !== 1 ? "s" : ""}
                          </span>
                          {reporter && (
                            <span className="admin-reporter-info">
                              <Users size={12} />{" "}
                              {reporter.fullName || reporter.phone || "Unknown"}
                            </span>
                          )}
                          {hasAutoDispatch && (
                            <span className="admin-dispatch-badge">
                              <Bell size={10} />{" "}
                              {incident.autoDispatch.volunteerName ||
                                "Auto-dispatched"}
                            </span>
                          )}
                        </div>
                          ...
                          <div className="admin-incident-detail">
                            ...
                            {reporter && (
                              <div className="admin-detail-section">
                                ...
                                <p>
                                  <strong>{reporter.fullName}</strong> &middot;{" "}
                                  {reporter.phone}
                                  {reporter.trustScore != null && (
                                    <span className="admin-trust-score">
                                      {" "}
                                      Trust: {reporter.trustScore}/10
                                    </span>
                                  )}
                                </p>
                              </div>
                              ...
                              <div className="admin-detail-section">
                                ...
                                {responders.map((r, idx) => {
                                  ...
                                  return (
                                    <p
                                      key={r._id || idx}
                                      className="admin-responder-line"
                                    >
                                      <UserCheck size={12} />
                                      <strong>
                                      ...
                                      <span className="admin-join-time">
                            ...
                            {hasAutoDispatch && (
                              <div className="admin-detail-section">
                                ...
                                <p>
                                  ...
                                  {incident.autoDispatch.volunteerPhone && (
                                    <>
                                      {" "}
                                      &middot;{" "}
                                      {incident.autoDispatch.volunteerPhone}
                                  ...
                                  {incident.autoDispatch.distanceKm != null && (
                                    <>
                                      {" "}
                                      &middot;{" "}
                                      {incident.autoDispatch.distanceKm.toFixed(
                                        2,
                                      )}{" "}
                                      km
                                    </>
                                  )}
                                  {incident.autoDispatch.rating != null && (
                                    <>
                                      {" "}
                                      &middot; Rating:{" "}
                                      {incident.autoDispatch.rating}
                                    </>
                                  )}
                                </p>
                                <p className="admin-dispatch-reason">
                                  {incident.autoDispatch.reason}
                                </p>
                              </div>
                            )}
                            ...
                            {incident.status === "open" && (
                              <div className="admin-action-row">
                                ...
                                <button
                                  ...
                                  onClick={() =>
                                    handleNotifyVolunteers(incident._id)
                                  }
                                >
                                  {actionLoading ===
                                  `notify-${incident._id}` ? (
        ...
        {activeTab === TAB_USERS && (
          <section className="admin-users-section">
            <div className="admin-users-header">
              <h3>
                <strong>User Management ({filteredUsers.length})</strong>
              </h3>
              ...
              >
                <RefreshCw
                  size={12}
                  className={usersLoading ? "spinning" : ""}
                />{" "}
                Refresh
              </button>
            </div>
                  ...
                  {filteredUsers.length === 0 ? (
                    ...
                    filteredUsers.map((user) => (
                      <tr
                        key={user._id}
                        className={
                          user.isSuspended ? "admin-user-suspended" : ""
                        }
                      >
                        <td>
                        ...
                        <td>
                          <span
                            className={`admin-trust-pill ${user.trustScore < 3 ? "admin-trust-pill--low" : user.trustScore >= 7 ? "admin-trust-pill--high" : ""}`}
                          >
                        ...
                        <td>
                          <div className="admin-skills-cell">
                            {(user.skills || []).slice(0, 3).map((s) => (
                              <span key={s} className="admin-skill-chip">
                                {s}
                              </span>
                            ))}
                        ...
                        <td>
                          {user.successfulResponses || 0}/
                          {user.totalResponses || 0}
                          {(user.falseAlertCount || 0) > 0 && (
                            <span className="admin-false-alert">
                              {" "}
                              ({user.falseAlertCount} false)
                            </span>
                          )}
                        </td>
                        <td>
        ...
        <footer className="ud-footer admin-footer">
          <p>
            <strong>FirstLine Admin</strong> &mdash; Centralized emergency
            command &amp; oversight
          </p>
        </footer>
      </section>
